...
// style: セルスタイルID
// rcvs: RCVモデルの配列
func update_sheet_next(f *excelize.File, files []model.ArParm, styles map[string]int) {
	...
	endcell := fmt.Sprintf("G%d", len(files)+4-1)
	// fmt.Println(endcell)
...
// // data: RCV詳細データの配列
// func update_sheet_next_def(f *excelize.File, data []string, styles map[string]int) {
// 	const sheet = "DefRcv"
// 	// 共通の詳細データ更新関数を呼び出す
